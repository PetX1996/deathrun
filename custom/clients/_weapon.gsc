...
onPlayerSpawned()
{
	...
	self thread CWatchWeaponReload();
}

// callback
private CMonitorWeaponChanging()
{
	self endon("death");
...
/*CCheckActionSlots(sNew)
{
	switch (sNew)
	{
		case C_ICPERK::PerkBtnWeapon: // PERK BUTTON
			args = IECALLBACK::Args_Create();
			self C_INIT::RunOnCPerkBtnPressed(args);
			return true;
		default:
			return false;
	}
...
CWatchWeaponReload()
{
	...
	while (true)
	{
		...
		args = IECALLBACK::Args_Create();
		args.sWeapon = self GetCurrentWeapon();
		self C_INIT::W_RunOnCStartedReloading(args);
	}
}
