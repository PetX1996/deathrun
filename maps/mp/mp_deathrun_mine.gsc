...
lol()
{
	...
	trigger waittill ("trigger" , player );
	if (IsDefined(level.jumpertrigger)) level.jumpertrigger delete();
    if (IsDefined(level.bouncetrigger)) level.bouncetrigger delete();
    if (IsDefined(level.snipertrigger)) level.snipertrigger delete();
	if (IsDefined(level.trigger)) level.trigger delete();
	if (IsDefined(level.trigger2)) level.trigger2 delete();
