...
main()
{
	...
	//ambientPlay("ambient");
	mapper\_music::PlayNow("ambient", "Jessie J  - Nobody's Perfect (Netsky Full Vocal Remix)");
...
self.origin = self getorigin();
			...
			iBHForce = self mapper\_bunnyhop::CGetForce();
			if (!IsDefined(iBHForce))
				iBHForce = 0;

			force += iBHForce;
				
			bounceFrom = (self.origin - vector_scal( anglesToForward( self.angles ), gravity )) - (0,0,40);
			bounceFrom = vectorNormalize( self.origin - bounceFrom );
			self mapper\_damage::CRepelForce(force, bounceFrom);
			self mapper\_damage::CRepelForce(force, bounceFrom);
			self mapper\_damage::CRepelForce(force, bounceFrom);
			wait 0.1;
...
ropedown()
{
	...
	while(1)
	{
		...
		{
			if( Distance( start, players[i].origin ) <= 128 && isAlive(players[i]) && !IsDefined(players[i].isSliding) )
			{
