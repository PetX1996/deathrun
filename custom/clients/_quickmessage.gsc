...
{
	if (args.sMenuName == C_ICMENU::GetMenu("quickcommands"))
	{
		QuickCommands(args.sResponse);
		return;
	}
	else if (args.sMenuName == C_ICMENU::GetMenu("quickstatements"))
	{
		QuickStatements(args.sResponse);
		return;
	}
	else if (args.sMenuName == C_ICMENU::GetMenu("quickresponses"))
	{
		QuickResponses(args.sResponse);
		return;
	}
	else if (args.sMenuName == C_ICMENU::GetMenu("quickplayer"))
	{
		QuickPlayer(args.sResponse);
		return;
	}
}
...
QuickPlayer(sResponse)
{
	switch (sResponse)
	{
		case "qplayer_3rdrear":
			if (IsAlive(self))
				self C_ICPLAYER::ChangeOrToggleThirdPerson(0);
			break;
		case "qplayer_3rdfront":
			if (IsAlive(self))
				self C_ICPLAYER::ChangeOrToggleThirdPerson(180);
			break;
		case "qplayer_3rdleft":
			if (IsAlive(self))
				self C_ICPLAYER::ChangeOrToggleThirdPerson(90);
			break;
		case "qplayer_3rdright":
			if (IsAlive(self))
				self C_ICPLAYER::ChangeOrToggleThirdPerson(270);
			break;
		case "qplayer_suicide":
			if (IsAlive(self))
				self Suicide();
			break;
		default:
			IEXCEPTION::InvalidOperationMsg("Unknown menu response '" + sResponse + "'");
	}
}