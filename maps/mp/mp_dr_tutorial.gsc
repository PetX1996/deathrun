...
main()
{
	...
	maps\createfx\mp_dr_tutorial_fx::main();
	maps\mp\_compass::setupMiniMap("compass_map_mp_dr_tutorial");
...
MakeTriggers()
{
	...
	for(i=15;i>0;i--)
	{
		if( isDefined(level.dr_gametype)) break;	
		if( i <= 1 ) thread selectgame(RandomInt(4));
...
selectgame(nro)
{
	ambientPlay("ambient_mp_dr_tutorial");
...
Start_user()
{
		...
		for(turn=1;turn<36;turn++)	
		{
			if (isDefined(level.jumper_turn[turn]) &&  isAlive(level.jumper_turn[turn]))
			{
				...
				if (turn == 1)
				{ 
					iprintlnbold( level.jumper_turn[turn].name + " ^1reached as first the end hall!");
...
stargametype()      
{
	...
	finaltype3 thread finaldragunov();
	
	finaltype4 = getEnt("door_type_4", "targetname");
	finaltype4 thread finalm60e4();	
