...
jumptrig = getent ("jump_trigger", "targetname");
	...
	for(;;)
	{
		...
		if (IsDefined(snipertrig)) snipertrig delete();
		if (IsDefined(knifetrig)) knifetrig delete();
		if (IsDefined(oldtrig)) oldtrig delete();
		if (IsDefined(jumptrig)) jumptrig delete();
		wait(0.05);
