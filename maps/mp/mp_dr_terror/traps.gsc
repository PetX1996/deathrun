...
bonus()
{
	...
	player braxi\_rank::giveRankXP( "bonus", 50 );
}
bonus2()
{
	...
	trigger waittill( "trigger", player );
	player braxi\_rank::giveRankXP( "bonus", 80 );
}
bonus3()
{
	...
	trigger waittill( "trigger", player );
	player braxi\_rank::giveRankXP( "bonus", 120 );
}
bonus5()
{
	trigger = getEnt( "xp5", "targetname" );
	
	trigger waittill( "trigger", player );
	player braxi\_rank::giveRankXP( "bonus", 100 );
}
bonus4()
{
	trigger = getEnt( "xp4", "targetname" );
...
final_doors()
{
	...
	special_trig = getent("final_special","targetname");
	
	snip_trig thread snip(bounce_trig,weapons_trig,knife_trig,special_trig);
	bounce_trig thread bounce(snip_trig,weapons_trig,knife_trig,special_trig);
	weapons_trig thread weapons(snip_trig,bounce_trig,knife_trig,special_trig);
	knife_trig thread knife(snip_trig,bounce_trig,weapons_trig,special_trig);
	special_trig thread special(snip_trig,bounce_trig,weapons_trig,knife_trig);
}
...
special(final1,final2,final3,final4)
{
	self waittill("trigger", player);
	...
	iprintlnbold("Player ^1"+player.name+" ^3open SPECIAL room!");
	...
	while(1)
	{
		porty_axis = getentarray("special_axis","targetname");
		raxis = randomint(porty_axis.size);
		porty_allies = getentarray("special_allies","targetname");
		rallies = randomint(porty_allies.size);
...
SaveMapTime()
{
	self api\maplogic\_em::CTouchEM();
}
...
out()
{
trig = getent("_out","targetname");
brush = getent("out","targetname");
trig onactivetrap();
brush movey(800,1);
wait 2;
}
out2()
{
trig = getent("_out2","targetname");
brush = getent("out2","targetname");
trig onactivetrap();
brush movez(800,1);
wait 2;
}
ha1()
{
	active = getent("_h1","targetname");
	a = getent("h1","targetname");
...
ha2()
{
	active = getent("_h2","targetname");
	a = getent("h2","targetname");
...
ha3()
{
	active = getent("_h3","targetname");
	a = getent("h3","targetname");
...
ha4()
{
	active = getent("_h4","targetname");
	a = getent("h4","targetname");
...
pa1()
{
	active = getent("_p1","targetname");
	a = getent("p1","targetname");
...
pa2()
{
	active = getent("_p2","targetname");
	a = getent("p2","targetname");
...
pa3()
{
	active = getent("_p3","targetname");
	a = getent("p3","targetname");
...
pa4()
{
	active = getent("_p4","targetname");
	a = getent("p4","targetname");
...
AddLife()
{
	trigger = getEnt( "life", "targetname" );
	trigger waittill( "trigger", player );
