...
Main()
{
	level._effect["mp_dr2_view/ghost1"] = LoadFX("mp_dr2_view/ghost1");

	thread PrepareEnt("t6", "t6t");

	TrapActivator(6);
	
	thread MoveEnt("t6", "t6t", 300);
}
...
MoveEnt(sBrush, sTrig, fSpeed)
{
	...
	PlayFXOnTag(level._effect["mp_dr2_view/ghost1"], eModel, "tag_origin");
